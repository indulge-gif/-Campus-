Page({
  ...
  toPage4() {
    ...
    const target = this.data.locations.find(loc => loc.name === query);
    if (target) {
      wx.navigateTo({
        url: `/pages/page6/page6?endLat=${target.latitude}&endLng=${target.longitude}&name=${target.name}`
      });
    } else {
  ...
  navigateToSchool() {
    wx.getLocation({
      ...
      success: (res) => {
        ...
        wx.navigateTo({
          url: `/pages/page6/page6?startLat=${res.latitude}&startLng=${res.longitude}&endLat=${latitude}&endLng=${longitude}&name=中央民族大学`
        });
      },
      fail: () => {
